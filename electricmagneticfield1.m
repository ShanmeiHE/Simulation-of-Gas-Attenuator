...
sin_electron_theta = electron_position_half(:,2)./electron_position_r;
cos_electron_theta = electron_position_half(:,1)./electron_position_r;
electron_velocity_r = electron_velocity(:,1).*cos_electron_theta +  electron_velocity(:,2).*sin_electron_theta;
    ...
    if length(J)>0  
        ...
        az = round(electron_position_half(:,3)/delta_z);
        a = ar + az*numberz + 1;
        Jr = J(:,1);
        Jz = J(:,2);
...
tic
    ...
    if electron_number > 0
        ...
        F_r = -e*gridE_r(a) - e*(-electron_velocity_z.*gridB_theta(a));
...
%{
        ...
        electron_position = electron_position_half + 1/2*delta_t*electron_velocity;
