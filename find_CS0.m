...
a = find(E_PE >= 0.05);
    ...
    x_el = t_el(1,:);
    y_el = t_el(2,:);
    sigma_el(a) = interp1(x_el,y_el,E_PE(a)) * sqr_a0 * 2; % times 2 for nitrogen molecules
b = find (E_PE < 0.05);
    x_el = t_sel(:,1);
    y_el = t_sel(:,2);
    sigma_el(b) = interp1(x_el,y_el,E_PE(b)*1000) * 10^(-20);
    sigma_el = sigma_el';
...
sigma_el(sigma_el<0) = 0;
sigma_ion(sigma_ion<0) = 0;
sigma_exc(sigma_exc<0) = 0;
end
