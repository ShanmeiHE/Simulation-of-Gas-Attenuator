function [ vxout,vyout,vzout,vout,N_collisionout,ncollisiontime,eiout,n_SC ] = collision_HS_internal( INDEX,N1,cellnum,vx,vy,vz,v,SCmax,FN,VC,dt,d,alpha_,N_collision,ei,w,zeta,m )
%collision process,HS model
ncollisiontime=0;
...
%                 disp('error');
            ...
            if randnum<SC/SCmax  %collide
                ...
                Ec=1/4*m*cr^2+eipart(sub1)+eipart(sub2);
                
                % sample Et
                flag1=0;
                        while flag1==0
                            Et=Ec*rand(1);
                            p=((zeta+1/2-w)/(3/2-w)*(Et/Ec))^(3/2-w)*((zeta+1/2-w)/(zeta-1)*(1-Et/Ec))^(zeta-1);
                            randnum=rand(1);
                            if randnum<p
                                flag1=1;
                            end
                ...
                flag2=0;
                        while flag2==0
                            ...
                            p=2^(zeta-2)*(etry/Ei)^(zeta/2-1)*(1-etry/Ei)^(zeta/2-1);
                            randnum=rand(1);
                            if randnum<p
                                flag2=1;
               ...
               eipart(sub1)=etry;
               eipart(sub2)=Ei-etry;
               cr_internal=2*sqrt(Et/m);
               ur=ur*cr_internal/cr;
               vr=vr*cr_internal/cr;
               wr=wr*cr_internal/cr;
               cr=cr_internal;
